...
//		Assert.assertEquals(3, generalMapper.selectPage(new Page<UserVO>(1,3), ew).size());
    ...
    public void paginationTest() {
        ...
        page.setCurrent(1);
        page.setSize(3);
        GeneralEntityWrapper<User> ew = new GeneralEntityWrapper<>();
        ew.setEntity(new User());
        List<User> list = generalMapper.selectPage(page, ew);
        Assert.assertEquals(3, list.size());
        System.out.println(page);
        Assert.assertTrue(page.getTotal() != 0);

        Page page2 = new Page(1,3);
        List<User> list2 = userMapper.selectMyPage(page2);
        Assert.assertEquals(3, list2.size());
        System.out.println(page2);
        Assert.assertTrue(page2.getTotal() != 0);
    }
